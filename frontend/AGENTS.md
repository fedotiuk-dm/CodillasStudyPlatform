...
- Lint/format: **Biome** (`pnpm lint` / `pnpm format`). Package manager: **pnpm** only, pinned by
  `packageManager` in `package.json` (an older global pnpm auto-switches to it).
...
- **No client-side sorting.** Render API data in the order it arrives — no `.sort()`/`toSorted()`
  on server data. A new order → a named `Sort` on the backend repository or the `sort` param of a
  paginated endpoint. No URL-state libs (nuqs removed); sort/filter state goes to the query params.
