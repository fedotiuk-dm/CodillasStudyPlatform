...
3. **Repository** — `@Repository public interface … extends JpaRepository<Entity, UUID>` + derived
   ...
   `Sort.by("magicString")` in a service. Shorten long derived names with `Sort`/`Limit` params and
   the `First`/`Top` keyword (limiting query results), not `@Query`.
4. **Domain entity** — `@Entity @Getter @Setter @NoArgsConstructor @SuperBuilder`, extends the
   shared auditable base. `@Builder.Default` for collections/defaults. **Entities stay anemic**:
   no behaviour, no exception deps. A non-trivial status lifecycle lives in a separate **data-driven
   state machine** bean (`<Aggregate>StateMachine` with an allowed-transitions map → `ConflictException`
   on an illegal move), unit-tested in isolation. Entities needing manual ordering implement
   `shared.domain.Sortable` and their repository extends `shared.domain.repository.SortableRepository`
   (`getNextSortOrder()`).
...
- **CHECK constraints** (`ck_<table>_<what>`, since 0.7.0) keep impossible rows out regardless of
  which writer produced them. Liquibase OSS has no `addCheckConstraint`/`checkConstraintExists`
  (Pro-only), so they use raw `sql:` with an inline `rollback:` attribute and a `sqlCheck`
  precondition against `pg_constraint`. One `ALTER TABLE` per table (Postgres validates all its
  added constraints in a single scan), not one per constraint. A constraint listing an enum's
  values must match the Java enum exactly — `EnumCheckConstraintTest` in `main` asserts this, so
  adding a constant without touching the changelog fails the build. **Never** CHECK an enum column
  on an event-fed projection of another module's vocabulary: an unknown value must degrade to an
  unmatched row, not a poison message in the event-publication registry.
- The **Hibernate static metamodel** (`Entity_`) is generated by `hibernate-processor` (wired in the
  parent pom's compiler `annotationProcessorPaths`) — use `Entity_.FIELD` constants for type-safe
  `Sort`/Specification field names instead of string literals.
