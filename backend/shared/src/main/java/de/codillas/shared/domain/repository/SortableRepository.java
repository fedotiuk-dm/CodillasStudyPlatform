package de.codillas.shared.domain.repository;

import org.springframework.data.jpa.repository.JpaSpecificationExecutor;
import org.springframework.data.repository.NoRepositoryBean;

import de.codillas.shared.domain.Sortable;

/**
 * Repository mixin for entities that support {@code sortOrder}. Add {@code
 * SortableRepository<MyEntity>} to a repository's extends clause — the entity must implement {@link
 * Sortable} — to get {@link #getNextSortOrder()} for auto-assigning the next free slot.
 *
 * @param <T> entity type that implements {@link Sortable}
 */
@NoRepositoryBean
public interface SortableRepository<T extends Sortable> extends JpaSpecificationExecutor<T> {

  /** Returns the next available sortOrder (current max + 1, starting from 1). */
  default int getNextSortOrder() {
    return GenericSpecification.getNextSortOrder(this);
  }
}
